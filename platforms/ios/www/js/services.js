...
.factory('Profile', function($rootScope, $cordovaSQLite) {
  ...
  return {
    ...
    setAll: function(profile, when) {
      ...
      $cordovaSQLite.execute($rootScope.db, updateBio, [(profile.user_name != '' && profile.user_name != 0) ? profile.user_name : "0", (profile.user_surname != '' && profile.user_surname != 0) ? profile.user_surname : "0", (profile.user_middlename != '' && profile.user_middlename != 0) ? profile.user_middlename : "0", (profile.user_email != '' && profile.user_email != 0) ? profile.user_email : "0", (profile.user_mob_no_code != '' && profile.user_mob_no_code != 0) ? profile.user_mob_code+''+profile.user_mob_no_code : "0", (profile.user_pic != '' && profile.user_pic != 0) ? profile.user_pic : "0", (profile.user_gender != '' && profile.user_gender != 0) ? profile.user_gender : "0", profile.user_birthday, (profile.user_adress != '' && profile.user_adress != 0) ? profile.user_adress : "0", when, 1]).then(function() {}, function(er) {});
    }
  };
...
.factory('Waves', function($rootScope, $cordovaSQLite) {
  
  return {
    all: function() {
      var waves = [];
      var queryWave = "SELECT * FROM waves WHERE wave_del = ? ORDER BY wave_id DESC LIMIT 1";
      return $cordovaSQLite.execute($rootScope.db, queryWave, [0]).then(function(suc) {
          if(suc.rows.length > 0) {
            for(var i=0;i<suc.rows.length;i++) {
              waves.push(suc.rows.item(i));
            }
            return waves;
          }
          else {
    ...
    get: function(user, priority) {
      var waves = [];
      var queryWave = "SELECT * FROM waves WHERE wave_user = ? AND wave_priority = ? AND wave_del = '0' ORDER BY wave_id DESC LIMIT 1";
      return $cordovaSQLite.execute($rootScope.db, queryWave, [user, priority]).then(function(suc) {
          if(suc.rows.length > 0) {
            for(var i=0;i<suc.rows.length;i++) {
              waves.push(suc.rows.item(i));
            }
