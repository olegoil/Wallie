...
.controller('LoginCtrl', function($scope, $state) {
  ...
  // var anotherSeed = Waves.Seed.fromExistingPhrase('olegtronics is comming today and tonight');
  // console.log(seed.phrase);
  // console.log(seed.address);
  // console.log(seed.keyPair);
  // var password = '0123456789';
  // var encrypted = seed.encrypt(password);
  // console.log(encrypted);
  // var restoredPhrase = Waves.Seed.decryptSeedPhrase(encrypted, password);
  ...
  //     timestamp: Date.now()
  // };
  // aves.API.Node.v1.assets.transfer(transferData, seed.keyPair).then((responseData) => {
  //     console.log(responseData);
  // });
  // var address = Waves.tools.getAddressFromPublicKey('3PMgh8ra7v9USWUJxUCxKQKr6PM3MgqNVR8');
  // console.log(address); // '3N1JKsPcQ5x49utR79Maey4tbjssfrn2RYp'

})

.controller('TabCtrl', function($scope, $state, $stateParams, $ionicHistory, $cordovaGlobalization, $translate, $http, Cart, Profile, Points, HtmlEnt) {
  ...
  var jsoncontr = JSON.stringify({
    curstate: 1,
    id: 1
  });

  $http.post('http://www.communiloca.com/admin/constructor/controller.php', jsoncontr).then(function(suc) {
    var des = suc.data;
    des[0].constArr[0].Map == '1' ? $scope.map = true : $scope.map = false;
    des[0].constArr[0].Menu == '1' ? $scope.menu = true : $scope.menu = false;
    des[0].constArr[0].Discount == '1' ? $scope.discount = true : $scope.discount = false;
    des[0].constArr[0].Contacts == '1' ? $scope.contacts = true : $scope.contacts = false;
  }, function(er) {
...
.controller('CardCtrl', function($rootScope, $scope, $state, $stateParams, $ionicHistory, $ionicModal, $ionicPopup, $timeout, $translate, $http, Cart, Survey, Profile, Points, Waves) {
  ...
  $scope.$on('$ionicView.enter', function() {
    ...
    // $scope.phrase = '375293803734 ' + $rootScope.uuid;
    $scope.phrase = 'raccoon clap nose mule obey person math plate super enter sauce crew finish wink assist';
    Waves = WavesAPI.create(WavesAPI.TESTNET_CONFIG);
    seed = Waves.Seed.fromExistingPhrase($scope.phrase);
    var seedtoken = 1;
    $scope.seedphrase = seed.phrase;
    $scope.seedaddress = seed.address;
    $scope.seedkeypair = seed.keyPair;
    $scope.publicKey = $scope.seedkeypair.publicKey;
    $scope.privateKey = $scope.seedkeypair.privateKey;
    $scope.password = '375293803734';
    $scope.encrypted = seed.encrypt($scope.password);
    
    // TX ID: 68bC4TVR7AQXwJ4yL9CpYgrCUpt8bs28jNokjmRVYGTD | TYPE: Outgoing Asset Issue | DATE: 10/09/2017 16:57:12 | SENDER ADDRESS: 3MrjzRTcMZwdzzMD1sZPdAqNESMaGMqdt1v | TX AMOUNT: 1000000.000 mymoney | RECIPIENT ADDRESS: n/a | TX FEE: 1 Waves
    $timeout(function() {
      Waves.API.Node.v1.addresses.balance($scope.seedaddress).then(function(balance) {
        // console.log('Balance '+JSON.stringify(balance));
        $scope.balance = balance.balance;
      }, function(er) {console.log('Error '+JSON.stringify(er))});
    }, 200)
  ...
  $scope.sendMoney = function() {
    var transferData = {
        ...
        // An arbitrary address; mine, in this example
        recipient: '3MpX6n6ZESnrjwr2EumcXc2pTWfS67KNuza',
      ...
      Waves.API.Node.v1.assets.transfer(transferData, seed.keyPair).then(function(responseData) {
  ...
  $scope.$on('$ionicView.afterEnter', function() {
    ...
    $http.post('http://www.communiloca.com/admin/constructor/controller.php', jsoncontr).then(function(suc) {
      ...
      des[0].constArr[0].Basket == '1' ? $scope.basket = true : $scope.basket = false;
      des[0].constArr[0].Gifts == '1' ? $scope.gifts =  true : $scope.gifts = false;
      des[0].constArr[0].Community == '1' ? $scope.community = true : $scope.community = false;
      des[0].constArr[0].Share == '1' ? $scope.share = true : $scope.share = false;
      des[0].constArr[0].Banner == '1' ? $scope.banner = true : $scope.banner = false;
      des[0].constArr[0].Map_of_client == '1' ? $scope.mapofclient = true : $scope.mapofclient = false;
    }, function(er) {
      console.log(JSON.stringify(er));
    });
...
.controller('ElseCtrl', function($rootScope, $scope, $state, $stateParams, $ionicHistory, $ionicModal, $ionicPopup, $ionicActionSheet, $cordovaSQLite, $cordovaDevice, $timeout, $translate, $ionicLoading, $http, $cordovaNetwork, $cordovaCamera, $cordovaFile, $cordovaFileTransfer, $cordovaImagePicker, $cordovaBarcodeScanner, Inst, Cart, Bill, Profile, HtmlEnt, FAQ) {
  ...
  $scope.sendPromo = function() {
    ...
    $http.post($rootScope.generalscript, promostr).then(function(e) {
        ...
        else if(promocodeOK == $scope.profile.user_promo) {
...
.controller('ProfileEditCtrl', function($rootScope, $scope, $state, $stateParams, $ionicHistory, $ionicActionSheet, $ionicModal, $ionicPopup, $ionicLoading, $http, $cordovaSQLite, $cordovaDevice, $timeout, $cordovaNetwork, $cordovaCamera, $cordovaFile, $cordovaFileTransfer, $cordovaImagePicker, $translate, $ionicPickerI18n, ionicDatePicker, Profile, Phone, HtmlEnt, timezoneAdd, timezoneSub) {
  ...
  $scope.createWaves = function(phrase) {

    $scope.phrase = 'raccoon clap nose mule obey person math plate super enter sauce crew finish wink assist';
    var Waves = WavesAPI.create(WavesAPI.TESTNET_CONFIG);
    // var seed = Waves.Seed.create();
    var seed = Waves.Seed.fromExistingPhrase($scope.phrase);
    var seedtoken = 1;
    $scope.seedphrase = seed.phrase;
    $scope.seedaddress = seed.address;
    var seedkeypair = seed.keyPair;
    $scope.publicKey = seedkeypair.publicKey;
    $scope.privateKey = seedkeypair.privateKey;
    $scope.password = '375293803734';
    $scope.encrypted = seed.encrypt($scope.password);
    $scope.phrase = phrase;

    var wavestr = JSON.stringify({
      ...
      action: 'set',
      seedtoken: seedtoken,
      seedphrase: $scope.seedphrase,
      seedaddress: $scope.seedaddress,
      publicKey: $scope.publicKey,
      privateKey: $scope.privateKey,
      pass: $scope.encrypted
    });

    // $timeout(function() {
    //   $scope.closeModal(5);
    //   $timeout(function() {$state.go('tab.card');}, 100);
    // }, 500);

    $http.post($rootScope.generalscript, wavestr).then(function(wavesuc) {
      
      var wavesd = wavesuc.data[0].wavesArr;
      // console.log(JSON.stringify(wavesd))
      if(wavesd.length > 0) {
        var waveIns = "INSERT INTO waves (wave_id, wave_user, wave_publicKey, wave_privateKey, wave_seedPhrase, wave_seedAddress, wave_pass, wave_institution, wave_priority, wave_when, wave_del) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
        $cordovaSQLite.execute($rootScope.db, waveIns, [wavesd.wave_id, wavesd.wave_user, $scope.publicKey, $scope.privateKey, $scope.seedphrase, $scope.seedaddress, $scope.encrypted, $rootScope.institution, 0, wavesuc.wave_when, 0]).then(function() {
          // $scope.closeModal(5);
          // $timeout(function() {$state.go('tab.card');}, 100);
        }, function() {});
      }
  ...
  $scope.smsConfirm = function() {
    ...
    $scope.createWaves($scope.profile.user_mob + ' ' + $scope.profile.user_device_id);
