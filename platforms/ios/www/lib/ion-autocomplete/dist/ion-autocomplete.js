    ...
    function ($ionicBackdrop, $ionicScrollDelegate, $document, $q, $parse, $interpolate, $ionicPlatform, $compile, $templateRequest) {
        return {
            ...
            controller: ['$attrs', '$timeout', '$scope', function ($attrs, $timeout, $scope) {
                ...
                $timeout(function () {
                    controller.placeholder = valueOrDefault(controller.placeholder, 'Нажать чтоб ввести...');
                    controller.cancelLabel = valueOrDefault(controller.cancelLabel, 'Принять');
                    controller.selectItemsLabel = valueOrDefault(controller.selectItemsLabel, "Выберите...");
                    controller.selectedItemsLabel = valueOrDefault(controller.selectedItemsLabel, $interpolate("Выбрано items{{maxSelectedItems ? ' (max. ' + maxSelectedItems + ')' : ''}}:")(controller));
            ...
            link: function (scope, element, attrs, controllers) {
                ...
                var template = [
                    ...
                    // '<button class="ion-autocomplete-cancel button button-assertive" ng-click="viewModel.cancelClick()">{{viewModel.cancelLabel}}</button>',
                    '<button class="ion-autocomplete-cancel button icon ion-close-circled assertive" ng-click="viewModel.cancelClick()"></button>',
                    '</div>',
                    ...
                    '<ion-item collection-repeat="item in viewModel.searchItems track by $index" item-height="55px" item-width="100%" ng-click="viewModel.selectItem(item)" class="item-text-wrap">',
